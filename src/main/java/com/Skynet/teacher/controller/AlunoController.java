...
public class AlunoController {
	...
	public ResponseEntity<?> deleteAluno(@RequestBody Presenca presenca) {
		try {
			Boolean aulaDeletada = alunoServ.deletarPresenca(presenca.getAluno_id(), presenca.getAula_id());
			if (!aulaDeletada) {
				return new ResponseEntity<String>("A Presença não pode ser deletada por não existir, ou por algum erro interno",HttpStatus.UNPROCESSABLE_ENTITY);
			}
